...
import { Trash2, UserPlus, Shield, User, X, Briefcase, TrendingUp, BadgeCheck, Copy, Check, Loader2, AlertCircle, Search, Edit2, ChevronDown, Menu, Unlock } from 'lucide-react';
...
export default function UsersPage() {
    ...
    const handleUnlock = async (userId) => {
        if (!window.confirm("Unlock this user's security PIN?")) return;

        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/security/admin/unlock`, {
                method: 'POST',
                headers: {
                                            ...
                                            <tr key={member._id} className="hover:bg-gray-50 transition-colors group">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button onClick={() => handleUnlock(member._id)} className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-all" title="Unlock Security PIN"><Unlock className="w-5 h-5" /></button>
